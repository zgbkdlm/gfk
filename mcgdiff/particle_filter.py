"""
As per the license terms, here shows the modifications of the original code:

- Removed the adaptive time steps for precise ablation.
- Removed DDIM for precise ablation.
- Deleted `epsilon_predicted` from the function return, as this is never used.
- Used Ctrl + Alt + L in PyCharm to format the code.

Notes
-----
In all my implementations, the time always flows forward. In this implementation, the time is adapted to flow backward.
Similarly, the time is also adapted to be discrete.
"""
from typing import Tuple, List

import torch
from torch.distributions import Categorical
from mcgdiff.sgm import ScoreModel, generate_coefficients_ddim
from mcgdiff.utils import get_taus_from_singular_values
...
def predict(score_model: ScoreModel,
            ...
            t_prev: float,
            eta: float,
            n_samples_per_gpu: int = 1) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if hasattr(score_model.net, 'device_ids'):
        ...
        for batch_idx in range(n_batches):
            epsilon_predicted.append(
                score_model.net(particles[batch_size * batch_idx:(batch_idx + 1) * batch_size], t).cpu())
...
def predict_sde_euler(rev_drift: Callable, rev_dispersion: Callable,
                      particles: torch.Tensor,
                      t: float, t_prev: float, T: float):
    fwd_tk = T - t_prev
    fwd_tkm1 = T - t
    mean = particles + rev_drift(particles, fwd_tkm1) * (fwd_tk - fwd_tkm1)
    noise = rev_dispersion(particles, fwd_tkm1) * (fwd_tk - fwd_tkm1) ** 0.5
    return mean, noise
...
def mcg_diff(
    ...
    :param disc_timesteps: The timesteps to be used for the diffusion generation
    ...
    filtering_timesteps = disc_timesteps[taus_indices.min().item():]
    propagation_timesteps = disc_timesteps[:taus_indices.min().item() + 1]

    pbar = enumerate(zip(filtering_timesteps.tolist()[1:][::-1],
                         filtering_timesteps.tolist()[:-1][::-1]))

    for i, (t, t_prev) in pbar:
        predicted_mean, predicted_noise = predict(score_model=score_model,
                                                  particles=particles,
                                                  t=t,
                                                  t_prev=t_prev,
                                                  eta=eta,
        ...
        previous_log_likelihood = gauss_loglik(
            ...
            diag_std=(1 - (1 - gaussian_var) * (
                    alphas_cumprod[t] / alphas_cumprod[taus[previously_active_coordinates_in_obs]])) ** .5)
        log_integration_constant = gauss_loglik(
            x=predicted_mean[:, active_coordinates_in_x],
            mean=rescaled_observations[active_coordinates_in_obs] * (
                    (alphas_cumprod[t_prev] / alphas_cumprod[taus[active_coordinates_in_obs]]) ** .5),
            diag_std=(predicted_noise ** 2 + 1 - (1 - gaussian_var) * (
                    alphas_cumprod[t_prev] / alphas_cumprod[taus[active_coordinates_in_obs]])) ** .5
        )
        ...
        Kprev = (predicted_noise ** 2 / (predicted_noise ** 2 + 1 - (1 - gaussian_var) * (
                alphas_cumprod[t_prev] / alphas_cumprod[taus[active_coordinates_in_obs]])).clip(1e-10, 1e10))
        new_particles = particles.clone()
        new_particles[:, inactive_coordinates_in_x] = z[:, inactive_coordinates_in_x] * predicted_noise + \
                                                      predicted_mean[ancestors][:, inactive_coordinates_in_x]
        new_particles[:, active_coordinates_in_x] = Kprev * rescaled_observations[active_coordinates_in_obs][None,
                                                            :] * ((alphas_cumprod[t_prev] / alphas_cumprod[
            taus[active_coordinates_in_obs]]) ** .5) + \
                                                    (1 - Kprev) * predicted_mean[ancestors][:,
                                                                  active_coordinates_in_x] + \
                                                    ((1 - (1 - gaussian_var) * (alphas_cumprod[t_prev] / alphas_cumprod[
                                                        taus[active_coordinates_in_obs]])) * Kprev) ** .5 * z[:,
                                                                                                            active_coordinates_in_x]
...
def mcg_diff_mod(
    ...
    """Modified MCGdiff for the synthetic experiments.
    """
    # Initialization
    T = ts[-1]
    n_particles, dim = initial_particles.shape
    ...
    for i, (t, t_prev) in pbar:
        predicted_mean, predicted_noise = predict_sde_euler(rev_drift, rev_dispersion,
                                                            particles=particles,
                                                            t=t,
                                                            t_prev=t_prev, T=T)
        active_coordinates_in_obs = torch.where(t_prev >= taus)[0]
    ...
    if len(propagation_timesteps) > 1:
        ...
        for i, (t, t_prev) in pbar:
            predicted_mean, predicted_noise = predict(rev_drift, rev_dispersion,
                                                      particles=particles,
                                                      t=t,
                                                      t_prev=t_prev, T=T)
            z = torch.randn_like(particles)
