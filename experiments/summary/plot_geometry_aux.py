...
def sampler(key_, obs_op_, obs_cov_, vs_, init, target):
    ...
    *_, rev_drift, rev_dispersion = make_gm_bridge(ws_, ms_, eigvals_, eigvecs_, a, b, t0, T)
    smc, _ = make_fk_normal_likelihood(obs_op_, obs_cov_, rev_drift, rev_dispersion,
                                       ...
                                       ts, mode='guided')
    samples_, log_ws_, esss_ = smc(key_, init, vs_, nparticles, stratified, 0.7, True)
...
ws, ms, covs, obs_op, obs_cov = generate_gm(subkey, dx, dy, 10,
                                            full_obs_cov=True, noiseless=False)
...
plt.savefig('geometry_aux.pdf', transparent=True, bbox_inches=Bbox([[4.1, 1.7], [12.2, 7.8]]))
plt.show()
