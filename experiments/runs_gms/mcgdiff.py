...
from gfk.feynman_kac import make_noiseless_mcgdiff
...
# The sampler
@jax.jit
def sampler(key_, obs_op_, y_, init, target):
    ws_, ms_, eigvals_, eigvecs_ = target
    *_, rev_drift, rev_dispersion = make_gm_bridge(ws_, ms_, eigvals_, eigvecs_, a, b, t0, T)
    smc = make_noiseless_mcgdiff(obs_op_, rev_drift, rev_dispersion, alpha, y_, ts)
    samples_, log_ws_, esss_ = smc(key_, init, nparticles, stratified, 0.7)
...
for k, key_mc in enumerate(keys_mc):
    ...
    ws, ms, covs, obs_op, obs_cov = generate_gm(subkey, dx, dy, args.ncomponents, noiseless=True)
    ...
    # Run the SMC sampler
    samples, log_ws, esss = sampler(key_algo, obs_op, y, init_particles, (ws, ms, eigvals, eigvecs))
